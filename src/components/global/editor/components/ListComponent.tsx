...
const ListItem: React.FC<ListItemProps> = ({ item, index, onChange, onKeyDown, isEditable, fontColor }) => (
  <input
    type="text"
    value={item}
...
export const TodoList: React.FC<ListProps> = ({ items, onChange, className, isEditable = true }) => {
  ...
  const handleChange = (index: number, value: string) => {
    if (isEditable) {
      const newItems = [...items];
      newItems[index] = `${items[index].startsWith("[x] ") ? "[x]" : "[ ]"} ${value}`;
      onChange(newItems);
    }
  };
        ...
        <li key={index} className="flex items-center space-x-2">
          ...
          <ListItem
            ...
            onChange={(index, value) =>
              handleChange(index, `${item.startsWith("[x] ") ? "[x] " : "[ ] "}${value}`)
            }
            onKeyDown={handleKeyDown}
            isEditable={isEditable}
            fontColor={currentTheme.fontColor}
